...
use teloxide::{
    dispatching::{update_listeners, Dispatcher},
    prelude::*,
...
pub async fn run(db_connection: PgConnection) {
    ...
    Dispatcher::builder(bot, handler)
        ...
        .dependencies(dptree::deps![Arc::new(Mutex::new(Db {
            conn: db_connection
        }))])
        .build()
        .setup_ctrlc_handler()
    ...
    match command {
        ...
        Command::Subscribe(tag) => {
            ...
            if conn.has_subscription(&tag, &user_id) {
                bot.send_message(
                    message.chat.id,
                    format!("You are already subscribed to {tag}"),
                )
                .await?;
            } else {
                conn.create_subscription(&tag, &user_id);
                log::info!("User {} subscribed to {}", user_id, tag);
                bot.send_message(message.chat.id, format!("Subscribed to {tag}"))
                    .await?;
            }
        }
        Command::Unsubscribe(tag) => {
            ...
            if conn.has_subscription(&tag, &user_id) {
                ...
                bot.send_message(message.chat.id, format!("Unsubscribed from {tag}"))
                    .await?;
            } else {
                bot.send_message(message.chat.id, format!("You're not subscribed to {tag}"))
                    .await?;
            }
        }
    };
